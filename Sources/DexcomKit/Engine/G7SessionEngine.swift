...
actor G7SessionEngine {
    ...
    /// engine concludes the session has probably ended.
    ...
    private func handleStateChange(_ state: CentralState) async {
        ...
        case .unavailable(let reason):
            Log.connection.error("Bluetooth unavailable: \(String(describing: reason), privacy: .public)")
            centralReady = false
            rescanTask?.cancel()
            rescanTask = nil
            currentPeripheral = nil
    ...
    private func beginScanning() async {
        ...
        // no scan latency, and it works from background wakes.
    ...
    private func handleDiscovery(
        ...
        guard isStarted else { return }
        // Already connecting or connected (possibly a pending connect to
        // this same peripheral) — ignore advertisements meanwhile.
        guard currentPeripheral == nil else { return }
    ...
    private func handleConnected(_ id: UUID) {
    ...
    private func handleValue(_ id: UUID, characteristic: G7Characteristic, data: Data) async {
        ...
        case .backfillFinished:
            Log.backfill.info("Backfill stream finished with \(self.backfill.count, privacy: .public) records")
            await flushBackfill()
        case .sessionStopped:
    ...
    private func handleGlucose(_ message: GlucoseMessage) async {
        guard let peripheral = currentPeripheral else { return }
        let name = currentSensorName ?? peripheral.name ?? "DXCM"
        ...
        if followedSensor?.name != name {
            ...
            sessionEndReported = false
            let session = SensorSession(
                sensorName: name, activationDate: activation, extendedVersion: nil)
            self.session = session
            await emit(.sessionEstablished(session))
        } else if var followed = followedSensor,
            followed.peripheralIdentifier != peripheral.identifier
        {
            // Same sensor, new peripheral identifier (e.g. after a restore
            // on another device profile) — keep the fast reconnect path
            // working.
            followed.peripheralIdentifier = peripheral.identifier
    ...
    private func flushBackfill() async {
        guard !backfill.isEmpty, let followed = followedSensor else { return }
    ...
    private func handleRestore(_ peripherals: [any PeripheralLink]) async {
        ...
        for peripheral in peripherals {
            ...
            let shouldAdopt = AdoptionPolicy.shouldConnect(
                advertisedName: name,
                selection: configuration.selection,
                followedSensorName: followedSensor?.name,
                sessionHasEnded: sessionEndReported
            )
            guard currentPeripheral == nil, shouldAdopt else { continue }
    ...
    private func scheduleRescan() async {
        ...
        Log.connection.debug("Rescanning in \(String(describing: Self.rescanDelay), privacy: .public)")
